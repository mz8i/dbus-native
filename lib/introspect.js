...
DBusInterface.prototype.$createProp = function(propName, propType, propAccess) {
  ...
  if (propAccess.indexOf('read') !== -1) {
    this['getProp' + propName] = () => this.$readPropPromised(propName);
  }

  // access == write or readwrite
  if (propAccess.indexOf('write') !== -1) {
    this['setProp' + propName] = val => this.$writePropPromised(propName, val);
  }
};
  ...
  bus.invoke(
    ...
    function(err) {
      if (err) {
        ...
        // TODO: implement waiting for PropertiesChanged signal
        setTimeout(() => callback(err, val), 0);
      }
    }
  );
};

DBusInterface.prototype.$readPropPromised = promisify(
  DBusInterface.prototype.$readProp
);
DBusInterface.prototype.$writePropPromised = promisify(
  DBusInterface.prototype.$writeProp
);
