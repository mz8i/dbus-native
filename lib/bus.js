...
module.exports = function bus(conn, opts) {
  ...
  function DBusService(name, bus) {
    this.name = name;
    this.bus = bus;
    this.getObject = promisify(function(name, callback) {
      if (name == undefined)
    ...
    this.getInterface = async function(objName, ifaceName) {
      let obj = await this.getObject(objName);
  ...
  this.getObject = async function(path, name) {
    var service = this.getService(path);
    return await service.getObject(name);
  };

  this.getInterface = async function(path, objname, name) {
    let obj = await this.getObject(path, objname);
    return obj.as(name);
  };
